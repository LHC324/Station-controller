...
/* USER CODE BEGIN EV */
extern osSemaphoreId Recive_Uart1Handle;
extern osSemaphoreId Recive_Uart2Handle;
extern osSemaphoreId Recive_Uart3Handle;
/* USER CODE END EV */
...
void USART1_IRQHandler(void)
{
  ...
  if ((__HAL_UART_GET_FLAG(&huart1, UART_FLAG_IDLE) != RESET))
  {
    ...
    if (Dwin_Object)
    {
      ...
      Dwin_Recive_Len(Dwin_Object) = Dwin_Rx_Size(Dwin_Object) - __HAL_DMA_GET_COUNTER(&hdma_usart1_rx);
...
void USART2_IRQHandler(void)
{
  ...
  if ((__HAL_UART_GET_FLAG(&huart2, UART_FLAG_IDLE) != RESET))
  {
    ...
    if (Slave2_Object)
    {
      ...
      /*Get the number of untransmitted data in DMA*/
      /*Number received = buffersize - the number of data units remaining in the current DMA channel transmission */
      mdRTU_Recive_Len(Slave2_Object) = MODBUS_PDU_SIZE_MAX - __HAL_DMA_GET_COUNTER(&hdma_usart2_rx);
      /*Reopen DMA reception*/
      HAL_UART_Receive_DMA(&huart2, mdRTU_Recive_Buf(Slave2_Object), MODBUS_PDU_SIZE_MAX);
    }
    /*After opening the serial port interrupt, the semaphore has not been created*/
    if (Recive_Uart2Handle != NULL)
    {
      /*Notification task processing*/
      osSemaphoreRelease(Recive_Uart2Handle);
    }
  }
#endif
...
void USART3_IRQHandler(void)
{
  ...
  if ((__HAL_UART_GET_FLAG(&huart3, UART_FLAG_IDLE) != RESET))
  {
    ...
    if (Slave1_Object)
    {
      ...
      /*Get the number of untransmitted data in DMA*/
      /*Number received = buffersize - the number of data units remaining in the current DMA channel transmission */
      mdRTU_Recive_Len(Slave1_Object) = MODBUS_PDU_SIZE_MAX - __HAL_DMA_GET_COUNTER(&hdma_usart3_rx);
      /*Reopen DMA reception*/
      HAL_UART_Receive_DMA(&huart3, mdRTU_Recive_Buf(Slave1_Object), MODBUS_PDU_SIZE_MAX);
    }
    /*After opening the serial port interrupt, the semaphore has not been created*/
    if (Recive_Uart3Handle != NULL)
    {
      /*Notification task processing*/
      osSemaphoreRelease(Recive_Uart3Handle);
    }
  }
